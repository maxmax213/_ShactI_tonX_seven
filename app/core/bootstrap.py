...
def _seed_course(db: Session, teacher: User, student: User) -> None:
    ...
    blocks_greeting = (
        db.query(Assignment)
        .filter(Assignment.lesson_id == lesson.id, Assignment.title == "Блоки: приветствие по имени")
        .first()
    )
    if blocks_greeting is None:
        payload = {
            "goal": "Соберите программу из блоков: спросите имя пользователя и выведите приветствие.",
            "hints": [
                "Используйте блок для ввода имени и два блока вывода.",
                "В блоке вывода текста можно написать только постоянную часть сообщения.",
            ],
            "allowed_blocks": [
                "input_variable",
                "print_text",
            ...
            "starter_blocks": [
                {"type": "comment", "params": {"text": "Сначала спросим имя"}},
        ...
        db.add(
            Assignment(
                lesson_id=lesson.id,
                title="Блоки: приветствие по имени",
                description="Соберите алгоритм из блоков и отправьте сгенерированный Python-код",
                assignment_type=AssignmentType.BLOCKS,
                max_score=100,
                is_auto_check=False,
