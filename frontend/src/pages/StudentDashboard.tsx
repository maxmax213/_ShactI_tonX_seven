...
export function StudentDashboard() {
      ...
      .then((tree) => {
        ...
        setExpandedModules((current) => {
          ...
          for (const module of tree.modules) {
            if (next[module.id] === undefined) next[module.id] = true;
          }
          return next;
        });
  ...
  useEffect(() => {
    ...
    if (firstTestAssignment) {
      api
        .assignment(firstTestAssignment.id)
        .then((assignment) => {
        ...
        .catch((err) => setMessage(`Не удалось открыть тест: ${String(err)}`));
    } else {
      setActiveTestAssignment(null);
      setTestQuestions([]);
      setTestAnswers({});
    ...
    if (firstPracticeAssignment) {
      api
        .assignment(firstPracticeAssignment.id)
        .then((assignment) => {
          setActivePracticeAssignment(assignment);
          const previous = latestSubmissionByAssignment.get(assignment.id);
          setPracticeSolution(previous?.solution_payload ?? defaultPracticeSolution(assignment));
        })
        .catch((err) => setMessage(`Не удалось открыть задание: ${String(err)}`));
    } else {
      setActivePracticeAssignment(null);
      setPracticeSolution("print('solution')");
    }
  }, [selectedLesson?.id, latestSubmissionByAssignment]);
  ...
  async function onSubmitPractice(event: FormEvent): Promise<void> {
    ...
    try {
      await api.submitAssignment(activePracticeAssignment.id, practiceSolution);
                      ...
                      <textarea
                        ...
                        placeholder={
                          activePracticeAssignment.assignment_type === "blocks"
                            ? "Опишите последовательность блоков или вставьте сгенерированный код"
