...
import type { Achievement, Course, CourseTree, Submission, User } from "../app/types";
...
export function TeacherDashboard() {
  const [activeTab, setActiveTab] = useState<"content" | "review" | "achievements">("content");
  ...
  useEffect(() => {
    ...
    api
      .assignmentSubmissions(selectedAssignmentId)
      .then(setAssignmentSubmissions)
      .catch((err) => setMessage(`Ошибка загрузки решений: ${String(err)}`));
    ...
    <>
      <SectionCard title="Панель учителя">
        <div className="chip-row">
          <button className={activeTab === "content" ? "chip active" : "chip"} onClick={() => setActiveTab("content")}>
            Курсы и контент
          </button>
          <button className={activeTab === "review" ? "chip active" : "chip"} onClick={() => setActiveTab("review")}>
            Проверка решений
          </button>
          <button
            className={activeTab === "achievements" ? "chip active" : "chip"}
          ...
          <SectionCard title="Создание контента">
            ...
            <form className="form-grid" onSubmit={onCreateAssignment}>
              ...
              <select
                ...
                onChange={(event) =>
                  setAssignmentForm((current) => {
                    const assignmentType = event.target.value as "python" | "blocks" | "test";
                    return {
                      ...current,
                      assignment_type: assignmentType,
                      is_auto_check: assignmentType === "test" ? true : current.is_auto_check,
                    };
                  })
                }
              >
              ...
              />
              <textarea
                placeholder='JSON теста, пример: {"questions":[{"id":"q1","type":"single_choice","prompt":"...","options":[{"id":"a","text":"..."},{"id":"b","text":"..."}],"correct":"a"}]}'
      ...
      {activeTab === "review" && (
        <div className="two-col">
          <SectionCard title="Выбор задания для проверки">
            {!selectedTree && <EmptyState message="Выберите курс" />}
            {selectedTree && (
              <div className="tree-view">
                {selectedTree.modules.map((module) => (
                  <div key={module.id} className="tree-node">
                      ...
                      <div key={lesson.id} className="tree-lesson">
                        ...
                        <div className="chip-row">
                          {lesson.assignments.map((assignment) => (
                            <button
                              key={assignment.id}
                              className={selectedAssignmentId === assignment.id ? "chip active" : "chip"}
          ...
          <SectionCard title="Решения учеников">
            {selectedAssignmentId === null && <EmptyState message="Выберите задание" />}
      ...
      {activeTab === "achievements" && (
        <div className="two-col">
          <SectionCard title="Создать ачивку">
          ...
          <SectionCard title="Выдать ачивку ученику">
